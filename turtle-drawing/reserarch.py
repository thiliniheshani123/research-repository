...
# creating a screen
scr = turtle.getscreen() # getting screen
scr.title("Flag of America") # setting the title
scr.bgcolor("green") # setting the background colour

#Set the turtle object and speed of the turtle

t = turtle.Turtle() # putting turtle.turlte as t
t.speed(50) # setting the t speed
t.penup() # t penup

#putting flag height and width
flag_ht = 250 
flag_wth = 475

# metioning starting points of the flag
x1 = -250
y1 = 120
...
stripe_wdt = flag_wth
#star size
star_size = 11 # putting star size 


def draw_rectangle(x, y, height, width, color): # draw the rectangle
    t.goto(x,y)
    t.pendown()
    t.color(color) # colour 
    t.begin_fill() # filling the colour
    t.forward(width) # forwarding width
    t.right(90)
    t.forward(height) # forwarding height
    t.right(90)
    t.forward(width)
    t.right(90)
    t.forward(height)
...
def star_shape(x,y,color,length) : # make star shape
    t.goto(x,y)
    t.setheading(0)
    t.pendown()
    t.begin_fill()
